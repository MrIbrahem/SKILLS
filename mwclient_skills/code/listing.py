...
class GeneratorList(List):
    ...
    def __next__(
        self,
    ) -> Union["page.Page", "image.Image", "Category"]:
        info = super().__next__()
        if info["ns"] == 14:
...
class PageList(GeneratorList):
    ...
    def __getitem__(
        self, name: Union[str, int, "page.Page"]
    ) -> Union["page.Page", "image.Image", "Category"]:
        return self.get(name, None)

    def get(
        self,
        name: Union[str, int, "page.Page"],
        info: Optional[Mapping[str, Any]] = None,
    ) -> Union["page.Page", "image.Image", "Category"]:
        ...
        if self.namespace != 0:
            full_page_name = (
                f"{self.site.namespaces[self.namespace]}:{name}"
            )  # type: Union[str, int, 'page.Page']
...
class PageProperty(List):
    ...
    def set_iter(self, data: Mapping[str, Any]) -> None:
        for p in data["query"]["pages"].values():
            if p["title"] == self.page.name:
                self._iter = iter(p.get(self.list_name, ()))
                return
        raise StopIteration
