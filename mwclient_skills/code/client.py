...
from . import errors
from . import listing
from ._types import Cookies, Namespace, VersionTuple
from .sleep import Sleeper, Sleepers
...
class Site:
    ...
    def post(
        self, action: str, *args: Tuple[str, Any], **kwargs: Any
    ) -> Dict[str, Any]:
            ...
            try:
                ...
                if stream.headers.get("x-database-lag"):
                    ...
                    if not retry_on_error:
                        stream.raise_for_status()
                    log.warning(
                        "Received %d response: %s. Retrying in a moment.",
                        stream.status_code,
                        stream.text,
                    )
                    toraise = "stream"
    ...
    def clientlogin(self, cookies: Optional[Cookies] = None, **kwargs: Any) -> Any:
        ...
        if status == "PASS":
            ...
            raise errors.LoginError(
                self, status, response["clientlogin"].get("message")
            )
    ...
    def upload(
        ...
        if file is not None:
            ...
            if (
                self.require(1, 20, raise_error=False)
    ...
    def users(
        self, users: Iterable[str], prop: str = "blockinfo|groups|editcount"
    ) -> listing.List:
        """
    ...
    def ask(self, query: str, title: Optional[str] = None) -> Iterable[Dict[str, Any]]:
        ...
        while offset is not None:
            results = self.raw_api(
                "ask", query=f"{query}|offset={offset}", http_method="GET", **kwargs
            )
